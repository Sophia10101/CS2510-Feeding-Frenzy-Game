...
class PlayerFish {
  ...
  PlayerFish(int x, int y, int radius, Color color, boolean facingRight, int velX, int velY) {
    this.x = x;
    this.y = y;
    this.radius = radius;
    this.color = color;
  ...
  WorldScene draw(WorldScene scene) {
    ...
    // Body with outline
    WorldImage body = new CircleImage(this.radius, OutlineMode.SOLID, this.color);
    WorldImage bodyOutline = new CircleImage(this.radius + outlineThickness, OutlineMode.OUTLINE, outlineColor);

    // Tail with outline
    WorldImage tailBase = new EquilateralTriangleImage(this.radius * 1.5, OutlineMode.SOLID, this.color);
    WorldImage tailOutline = new EquilateralTriangleImage(this.radius * 1.5 + outlineThickness, OutlineMode.OUTLINE, outlineColor);
  ...
  PlayerFish move(String key) {
    ...
    return new PlayerFish(x + velX, y + velY, radius, color, facingRight, velX, velY).applyFriction().wrapAround(800, 600);
  } 
  ...
  PlayerFish applyFriction() {
    ...
    return new PlayerFish(x + velX, y + velY, radius, color, facingRight, velX, velY).wrapAround(800, 600);
  }
  ...
  int reduceVelocity(int velocity, int friction) {
    ...
    else if (velocity < 0) return Math.min(0, velocity + friction);
    else return 0;
  }
  ...
  PlayerFish wrapAround(int width, int height) {
    ...
    return new PlayerFish(newX, newY, this.radius, this.color, this.facingRight, this.velX, this.velY);
  }


  // Grow the player fish
  PlayerFish grow(int increment) {
    return new PlayerFish(this.x, this.y, this.radius + increment, this.color, this.facingRight, velX, velY);
  }
}
...
class BackgroundFish {
  ...
  BackgroundFish(int x, int y, int radius, int speed, boolean direction, Color color) {
    this.x = x;
    this.y = y;
    this.radius = radius;
    this.speed = speed;
  ...
  BackgroundFish move() {
    if (this.direction) {
      return new BackgroundFish(this.x + this.speed, this.y, this.radius, this.speed, this.direction, this.color).wrapAround(800, 600);
    } else {
      return new BackgroundFish(this.x - this.speed, this.y, this.radius, this.speed, this.direction, this.color).wrapAround(800, 600);
    }
  }
  ...
  BackgroundFish wrapAround(int width, int height) {
    if (this.x > width) {
      return new BackgroundFish(0, this.y, this.radius, this.speed, this.direction, this.color);
    } else if (this.x < 0) {
      return new BackgroundFish(width, this.y, this.radius, this.speed, this.direction, this.color);
    } else if (this.y > height) {
      return new BackgroundFish(this.x, 0, this.radius, this.speed, this.direction, this.color);
    } else if (this.y < 0) {
      return new BackgroundFish(this.x, height, this.radius, this.speed, this.direction, this.color);
    } else {
      return this;
    }
  }
...
class MtLoFish implements ILoFish {
  ...
  public ILoFish addRandomFish(Random rand, int count) {
    if (count > 0) {
      int randomSize = rand.nextInt(40) + 5; // Random size between 5 and 50
      ...
      BackgroundFish newFish = new BackgroundFish(initialX, randomY, randomSize, speed, direction, randomColor);
...
class ConsLoFish implements ILoFish {
  ...
  public CollisionResult checkCollisions(PlayerFish player) {
    if (Math.sqrt(Math.pow(this.first.x - player.x, 2) + Math.pow(this.first.y - player.y, 2)) < (this.first.radius + player.radius)) {
      ...
      return new CollisionResult(new ConsLoFish(this.first, result.fishList), result.player, result.gameLost);
    }
  }
  ...
  public ILoFish addRandomFish(Random rand, int count) {
    if (count > 0) {
      int randomSize = rand.nextInt(50) + 10; // Random size between 10 and 30
      return new ConsLoFish(new BackgroundFish(rand.nextInt(800), rand.nextInt(600), randomSize, rand.nextInt(3) + 1, rand.nextBoolean(), new Color(rand.nextInt(256), rand.nextInt(256), rand.nextInt(256))), this.rest.addRandomFish(rand, count - 1));
    } else {
      return this;
    }
  }
...
class GameWorld extends World {
  ...
  public WorldScene makeScene() {
    ...
    scene = scene.placeImageXY(new RectangleImage(800, 600, OutlineMode.SOLID, Color.CYAN), 400, 300); // Cyan background
    if (!playerLost && this.time > 0 && this.backgroundFish.largestFish() >= this.player.radius) {
  ...
  public World onTick() {
    if (!playerLost) {
      this.player = this.player.applyFriction(); // Update the player's position based on its current velocity
      ...
      if (this.playerLost) {
        return new GameWorld(this.player, this.backgroundFish, this.time, true).endOfWorld("Game Over: You were eaten!");
      }

      if (this.backgroundFish.largestFish() < this.player.radius) {
        return new GameWorld(this.player, this.backgroundFish, this.time, false).endOfWorld("YOU WIN!");
      }
    ...
    return new GameWorld(this.player, this.backgroundFish, this.time, this.playerLost);
  }
  ...
  public WorldScene makeEndScene(String message) {
    WorldScene scene = new WorldScene(800, 600);
    scene = scene.placeImageXY(new RectangleImage(800, 600, OutlineMode.SOLID, Color.CYAN), 400, 300); // Cyan background
    scene = scene.placeImageXY(new TextImage(message, 32, FontStyle.BOLD, Color.RED), 400, 300);
    return scene;
  }
}
...
class ExamplesMyWorldProgram {
  ...
  GameWorld game = new GameWorld(player, backgroundFish.addRandomFish(new Random(1), 30), 1000, false); // Added a seed for predictable results
  ILoFish consLoFish = new ConsLoFish(new BackgroundFish(250, 200, 10, 2, true, Color.blue), new MtLoFish());

  boolean testBigBang(Tester t) {
    PlayerFish player = new PlayerFish(250, 200, 15, Color.black, true, 0, 0);
    ILoFish backgroundFish = new MtLoFish();
    backgroundFish = backgroundFish.addRandomFish(new Random(1), 30); // Added a seed for predictable results
  ...
  boolean testBackgroundFishWrapAround(Tester t) {
    ...
    return t.checkExpect(wrapped, new BackgroundFish(0, 200, 15, 5, true, Color.blue))
        && t.checkExpect(new BackgroundFish(-10, 200, 15, 5, false, Color.blue).wrapAround(800, 600),
            new BackgroundFish(800, 200, 15, 5, false, Color.blue))
        && t.checkExpect(new BackgroundFish(250, 610, 15, 5, true, Color.blue).wrapAround(800, 600),
            new BackgroundFish(250, 0, 15, 5, true, Color.blue))
        && t.checkExpect(new BackgroundFish(250, -10, 15, 5, true, Color.blue).wrapAround(800, 600),
            new BackgroundFish(250, 600, 15, 5, true, Color.blue));
  }

  boolean testILoFishAddRandomFish(Tester t) {
    ILoFish emptyList = new MtLoFish();
    ILoFish newList = emptyList.addRandomFish(new Random(1), 1); // Added a seed for predictable results
    return t.checkExpect(newList instanceof ConsLoFish, true);
  }

  boolean testILoFishMove(Tester t) {
    ILoFish fishList = new ConsLoFish(new BackgroundFish(250, 200, 15, 5, true, Color.blue), new MtLoFish());
    ILoFish movedList = fishList.move();
    return t.checkExpect(movedList, new ConsLoFish(new BackgroundFish(255, 200, 15, 5, true, Color.blue), new MtLoFish()));
  }
  ...
  boolean testILoFishLargestFish(Tester t) {
    ILoFish fishList = new ConsLoFish(new BackgroundFish(250, 200, 10, 5, true, Color.blue), new MtLoFish());
    return t.checkExpect(fishList.largestFish(), 10);
  }

  boolean testGameWorldMakeScene(Tester t) {
    WorldScene scene = new WorldScene(800, 600);
    WorldScene expectedScene = scene.placeImageXY(new RectangleImage(800, 600, OutlineMode.SOLID, Color.CYAN), 400, 300)
        .placeImageXY(
  ...
  boolean testGameWorldOnKeyEvent(Tester t) {
    GameWorld initialGame = new GameWorld(new PlayerFish(250, 200, 15, Color.black, true, 0, 0), consLoFish, 1000, false);
    return t.checkExpect(initialGame.onKeyEvent("left"), 
        new GameWorld(new PlayerFish(245, 200, 15, Color.black, false, -5, 0), consLoFish, 1000, false))
        && t.checkExpect(initialGame.onKeyEvent("right"), 
            new GameWorld(new PlayerFish(255, 200, 15, Color.black, true, 5, 0), consLoFish, 1000, false))
        && t.checkExpect(initialGame.onKeyEvent("up"), 
            new GameWorld(new PlayerFish(250, 195, 15, Color.black, true, 0, -5), consLoFish, 1000, false))
        && t.checkExpect(initialGame.onKeyEvent("down"), 
            new GameWorld(new PlayerFish(250, 205, 15, Color.black, true, 0, 5), consLoFish, 1000, false));
  }
  ...
  boolean testGameWorldOnTick(Tester t) {
    ...
    return t.checkExpect(game.onTick(), 
        new GameWorld(new PlayerFish(254, 204, 15, Color.black, true, 4, 4), consLoFish.move(), 999, false));
  }

  boolean testGameWorldWorldEnds(Tester t) {
    GameWorld gameLost = new GameWorld(new PlayerFish(250, 200, 15, Color.black, true, 0, 0), consLoFish, 1000, true);
    GameWorld gameWon = new GameWorld(new PlayerFish(250, 200, 25, Color.black, true, 0, 0), consLoFish, 1000, false);
    return t.checkExpect(gameLost.worldEnds(), 
        new WorldEnd(true, gameLost.makeEndScene("Game Over: You were eaten!")))
        && t.checkExpect(gameWon.worldEnds(), 
            new WorldEnd(true, gameWon.makeEndScene("YOU WIN!")));
  }
}